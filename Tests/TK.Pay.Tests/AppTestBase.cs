...
using TK.Pay.Migrations.Seed.Host;
using TK.Pay.Migrations.Seed.Tenants;
using TK.Pay.MultiTenancy;
using TK.Pay.Tests.TestDatas;
...
namespace TK.Pay.Tests
{
    ...
    /// Provides methods to easily work with <see cref="PayDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<PayTestModule>
    {
        ...
        protected void UsingDbContext(Action<PayDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<PayDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<PayDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<PayDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<PayDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PayDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<PayDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<PayDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<PayDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<PayDbContext, Task<T>> func)
        {
            T result;
