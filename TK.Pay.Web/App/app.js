...
appModule.config([
    ...
    function ($stateProvider, $urlRouterProvider, $qProvider) {
        ...
        $stateProvider.state('ordermanage', {
            'abstract': true,
            url: '/ordermanage',
            template: '<div ui-view class="fade-in-up"></div>'
        });

        if (abp.auth.hasPermission('Pages.Order.OrderManage.OrderList')) {
            $stateProvider.state('ordermanage.orderlist', {
                url: '/orderlist',
                templateUrl: '~/App/pay/order/orderlist/index.cshtml'
            });
        }

        if (abp.auth.hasPermission('Pages.Order.OrderManage.NotifyList')) {
            $stateProvider.state('ordermanage.notifylist', {
                url: '/notifylist',
                templateUrl: '~/App/pay/order/notifylist/index.cshtml'
            });
        }
