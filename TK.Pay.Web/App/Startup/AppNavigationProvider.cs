...
using TK.Pay.Web.Navigation;

namespace TK.Pay.Web.App.Startup
{
    /// <summary>
    ...
    public class AppNavigationProvider : NavigationProvider
    {
        public override void SetNavigation(INavigationProviderContext context)
        {
            context.Manager.MainMenu
                ...
                //订单管理
                .AddItem(new MenuItemDefinition(
                    PageNames.Pay.Order.OrderManage,
                    L("OrderManage"),
                    icon: "icon-wrench"
                    ).AddItem(new MenuItemDefinition(
                        PageNames.Pay.Order.OrderList,
                        L("OrderList"),
                        url: "ordermanage.orderlist",
                        icon: "icon-layers",
                        requiredPermissionName: AppPermissions.Pages_Order_OrderManage_OrderList
                        )
                    ).AddItem(new MenuItemDefinition(
                        PageNames.Pay.Order.NotifyList,
                        L("NotifyList"),
                        url: "ordermanage.notifylist",
                        icon: "icon-layers",
                        requiredPermissionName: AppPermissions.Pages_Order_OrderManage_NotifyList
                        )
                    )
                )
                //设备管理
                .AddItem(new MenuItemDefinition(
                    PageNames.Pay.Device.DeviceManage,
                    L("OrderManage"),
                    icon: "icon-wrench"
                    ).AddItem(new MenuItemDefinition(
                        PageNames.Pay.Order.OrderList,
        ...
        {
            return new LocalizableString(name, PayConsts.LocalizationSourceName);
        }
    }
}
